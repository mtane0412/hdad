 ...
 * レジストリの定義と backgrounds/<id>/index.html が1対1で対応していることを確認する。
 */
...
const backgroundsDir = resolve(import.meta.dirname, '../../backgrounds')
...
describe('背景レジストリ', () => {
  ...
  it('backgrounds ディレクトリのページとレジストリの背景が1対1で対応している', () => {
    const pageIds = readdirSync(backgroundsDir, { withFileTypes: true })
  ...
  it.each(backgrounds.map((background) => [background.id]))(
    ...
    (id) => {
      const pagePath = resolve(backgroundsDir, id, 'index.html')
