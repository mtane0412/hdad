...
describe('オーバーレイ用API', () => {
  it('GET /api/overlay/config は、正しいキーなら素材のURL付きの設定を返す', async () => {
    const { env } = 環境を作る()
    const { id } = await 画像をアップロードする(env)
    await 呼び出す(await 配信者のリクエスト(env, '/api/admin/config', { method: 'PUT', body: JSON.stringify({ triggers: [トリガー(id)] }) }), env)

    const response = await 呼び出す(new Request(`${サイト}/api/overlay/config?key=${発行済みのキー}`), env)

    expect(response.status).toBe(200)
    const body = (await response.json()) as { triggers: { media: { kind: string; url: string } }[] }
    expect(body.triggers[0]?.media).toEqual({ kind: 'image', url: `/api/media/${id}?key=${発行済みのキー}` })
    // 設定は変わりうるので、古い内容を使い回させない
    expect(response.headers.get('Cache-Control')).toBe('no-store')
  })

  it('GET /api/overlay/config は、キーが違えば401を返す', async () => {
    const { env } = 環境を作る()
    const response = await 呼び出す(new Request(`${サイト}/api/overlay/config?key=atezuppou`), env)
    expect(response.status).toBe(401)
