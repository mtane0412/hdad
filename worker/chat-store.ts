 ...
 * 注意: 配信の区切りは、通知に書かれた発生時刻ではなく受け取った時刻（now）で引く。
 * 配信中の区切りは同時に1つしかない（recordStreamOnline がほかを閉じる）ので、どちらで引いても選ばれる区切りは同じである。
 * 注意: 同じ発言について二度問い合わせても、どちらにも true を返す（message_id が一致する行なら書き込み済みでも初回として扱う）。
 * 入口はWebhookの1本だけになったが、Twitch自身が同じ通知を再送することがあるため、この冪等性は引き続き要る。
 * 再送で false を返してしまうと、1通目の処理が途中で失敗していた場合に、やり直しでアラートが鳴らなくなる。
 *
