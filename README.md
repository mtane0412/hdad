...
イベント種別には**チャットの発言**（`channel.chat.message`）も選べるので、「特定の人が発言したら音を鳴らす」「合言葉が出たらアラートを出す」といった設定ができます。発言を絞るには「ユーザー」の条件（発言者のTwitchのユーザー名）・「文面に含む言葉」の条件（本文にその言葉を含む発言に当てはまる。大文字小文字は区別しない部分一致）と、次の3つの条件を使います。

| 条件 | 当てはまる発言 |
|---|---|
| その配信で初めての発言 | その配信で最初にチャットした人の1通目。同じ人の2回目以降の発言では動かず、配信が変わればまた動きます。**配信していないあいだの発言は初回と判定しません**（テスト配信や配信前の雑談のたびに動いてしまうため） |
| このチャンネルで初めての発言 | 視聴者の記録（`viewers`。「視聴者の記録」の節を参照）が1行もない人の1通目。常連は当てはまらないので、「初めて来てくれた人にだけ挨拶する」ができます。**この記録を始める前から来ている常連も「初めて」と扱われます**（記録がないためです。運用を始めた直後は動作を控えめにしておくことをおすすめします） |
| 前の発言から空いた日数 | 最後の発言から指定した日数（1〜365日）以上空けて発言した人の1通目。「お久しぶりです」と声をかけるのに使います。このチャンネルで初めての人には当てはまりません（空いた日数が決まらないためです） |

イベント種別には**広告の開始**（`channel.ad_break.begin`）と**広告の終了**（`channel.ad_break.end`）も選べるので、「広告が入ることをチャットで知らせる」「広告が終わったら声をかける」ができます。文言には広告の長さを差し込む `{duration}`（秒）が使えます。「自動で入った広告か」の条件を足すと、自動広告だけ・手動で打った広告だけに絞れます（手で打った広告は配信者が自分で告知できるので、ふだん告知したいのは自動広告のほうです）。
