...
後ろの2つは視聴者の記録から判定するので、**人ごとに記録を削除すると、その人はまた「初めて」に戻ります**。アラートを出す動作はbotの接続と関係なく動きますが、**チャットに送る・AIに文面を作らせて送る・アナウンスを送る動作はbotを接続しているときだけ動きます**（送り主のアカウントが要るため）。未接続のときは `/triggers/` の画面で知らせます。
...
| トリガー（`/triggers/`） | 「イベント」と「条件」（報酬＝チャンネルポイントの交換のみ／ユーザー／文面に含む言葉＝チャットの発言のみ、部分一致／その配信で初めての発言・このチャンネルで初めての発言・前の発言から空いた日数＝いずれもチャットの発言のみ／自動で入った広告か＝広告の開始・終了のみ。同じ種類は1件まで。すべてを満たしたときだけ動く）と、そのとき行う動作を並べて保存する。一覧では1行の要約に折りたたまれ、押すと入力欄が開く（開くのは1件ずつ）。動作は「アラートを出す」（素材・表示時間・音量・文言）・「チャットに送る」（文言）・「AIに文面を作らせて送る」（AIへの指示）・「アナウンスを送る」（文言・色）の4種類で、1つだけでも複数でもよい（ただし「チャットに送る」と「AIに文面を作らせて送る」は同じ発言に2通返ってしまうので同時に選べない）。複数のトリガーが当てはまるイベントには、上にあるトリガーを使う |
...
このツールがLLMに文面を作らせる場面は4つあります（トリガーの動作 `aiChat` のチャットの文面・サイドスーパー・視聴者の人物像・配信のこれまでのあらすじ）。その呼び先を、管理画面（`https://<公開先のドメイン>/llm/`）で切り替えられます。
...
モデルは**用途ごとに2つ**指定します。「チャットの文面・サイドスーパー・人物像」は短い文をたくさん作るので軽いモデルを、「配信のあらすじ」は配信の発話と書き込みをまとめるので大きいモデルを選びます（小さいモデルでは、視聴者の書き込みを配信者がしたことのように書く・同じ言い回しを繰り返して長さの上限を超える、といった壊れ方が起きました）。
...
- モデル名は**提供元ごとに別に覚えます**（`@cf/meta/llama-3.1-8b-instruct-fp8` と `meta-llama/llama-3.1-8b-instruct` のように書き方が違うため）。切り替えて戻しても入力し直す必要はありません
- OpenRouter を選んでいるのに鍵が無ければ、**文面を作る場面で失敗します**（黙って Workers AI へは切り替わりません）。管理画面はその状態を開いた時点で知らせます
- 失敗（鍵が無い・残高が足りない・無料枠を使い切った）は、それぞれの失敗として記録に残ります（`/api/admin/stats/failures` の `alert-aichat-failed`・`stream-summary-failed`・`side-super-failed`・`viewer-summary-failed`）。固定の文言に黙って落とすことはしません
...
- 届いた通知は、記録に加えてアラートのトリガーにかけます。当てはまるトリガーが「アラートを出す」動作を持っていれば、アラートをオーバーレイへ押し出します（botの接続は要りません）。「チャットに送る」「AIに文面を作らせて送る」「アナウンスを送る」動作を持っていれば、botとしてチャットへ1通送ります（botが未接続なら何もしません）。同じ通知が再送されても二度は実行せず、失敗した場合もTwitchへは2xxを返して `collection_failures` に `alert-push-failed`・`alert-chat-failed`・`alert-announce-failed`・`alert-aichat-failed` として記録します（2xx以外だとTwitchが再送し、実行できていた場合に二重になるため）。広告の終了は Durable Object のアラームから実行するので、その予約の失敗は `ad-break-end-schedule-failed`、アラームが鳴ってからの失敗は `ad-break-end-failed` として記録します（予約を消したあとなので、再試行では取り返せません）
