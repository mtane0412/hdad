...
後ろの2つは視聴者の記録から判定するので、**人ごとに記録を削除すると、その人はまた「初めて」に戻ります**。アラートを出す動作はbotの接続と関係なく動きますが、**チャットに送る・アナウンスを送る動作はbotを接続しているときだけ動きます**（送り主のアカウントが要るため）。未接続のときは `/triggers/` の画面で知らせます。
