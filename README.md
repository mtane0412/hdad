...
配信画面の隅（左上・右上）に、いま何をしているかを**1行20文字まで・最大2行**で出し続けるテロップです。文言は配信中の文字起こし・視聴者の発言・配信のカテゴリとタイトルを材料に、Worker が cron（5分おき）で作って貯め、このオーバーレイが読み出して映します。利用には「デプロイ」の節の Twitchログインの設定が必要です。
...
| `position` | `left`（既定。左上）か `right`（右上）。寄せる向きと帯のアクセントの位置が変わる |
