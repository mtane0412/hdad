...
「AIに文面を作らせて送る」動作（`aiChat`）は、配信者が書いた指示と、相手の名前・発言の本文・その人の記録（`viewers` のメモ・初回と最後の発言日時・おおよその発言数・最後に見たバッジ）と、「このチャンネルで初めての発言か」「前の発言から何日空いたか」を材料に、Workers AI（`wrangler.jsonc` の `ai` バインディング。モデルは `worker/ai-chat.ts` の `MODEL`）へ文面を作らせて送ります。視聴者の記録を材料に渡せるのはチャットの発言のトリガーだけで、ほかのイベントでは指示とイベントの中身だけから作らせます。Webhookの応答を待たせるとTwitchが同じ通知を再送するため、**Twitchへ2xxを返してから**文面を作って送ります（`ExecutionContext.waitUntil`）。AIが作った文面が500文字（Twitchのチャット1通の上限）を超えたとき、AIが失敗したとき（Workers AI の無料枠は1日10,000 Neuronsで、00:00 UTC にリセットされます）は、**切り詰めたり固定文言に落としたりせず送るのをやめて** `collection_failures` に `alert-aichat-failed` として記録します。
