...
| `/speech/` | チャットの読み上げの設定（アプリ）。VOICEVOX に読ませるOBS用URLをコピーする |
...
Twitchのチャットを、配信に使うPCで動かした [VOICEVOX](https://voicevox.hiroshiba.jp/) に読み上げさせるページです。映すものは持たず、音だけを出します。チャットボックスと同じ匿名IRCで受け取るので、Twitchのトークンもオーバーレイ用キーも要りません（接続先のチャンネル名だけを公開API `/api/chat/channel` から受け取ります）。読み上げに Workers AI を使わないのは、発言のたびに無料枠を消費させないためです。

```text
https://hdad.<サブドメイン>.workers.dev/speech/reader/?speaker=3&readName=true
```

| パラメータ | 内容 |
| --- | --- |
| `host` | VOICEVOX が動いているホスト。`localhost`（既定）か `127.0.0.1` のみ |
| `port` | VOICEVOX ENGINE のポート番号（既定 `50021`） |
| `speaker` | 話者ID（既定 `3` = ずんだもんのノーマル）。VOICEVOX のキャラクターとスタイルの組み合わせ |
| `speed` | 読み上げ速度（`0.5`〜`2`。既定 `1`） |
| `volume` | 音量（`0`〜`1`。既定 `1`） |
| `maxLength` | 読み上げる本文の長さの上限（文字数。`1`〜`200`。既定 `60`） |
| `readName` | `true` で本文の前に発言者の表示名を読む（既定 `false`） |
| `ignore` | 読み上げない人のログイン名（カンマ区切り）。`/speech/` の画面では、接続しているbotのログイン名が自動で入ります |
...
- 読まないもの: コマンド（`!` で始まる発言）・エモートやCheermoteだけの発言・`ignore` に挙げた人の発言
