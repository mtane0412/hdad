...
イベント種別には**チャットの発言**（`channel.chat.message`）も選べるので、「特定の人が発言したら音を鳴らす」「合言葉が出たらアラートを出す」といった設定ができます。発言を絞るには「ユーザー」の条件（発言者のTwitchのユーザー名）と「文面」の条件（本文に含まれる文字。大文字小文字を区別しない部分一致）を使います。アラートを出す動作は、オーバーレイが配信者のトークンで発言を購読するのでbotの接続と関係なく動きますが、**チャットに送る・アナウンスを送る動作はbotを接続しているときだけ動きます**（Workerが発言を受け取る購読の条件に、botのユーザーIDが要るため）。未接続のときは `/triggers/` の画面で知らせます。
...
| トリガー（`/triggers/`） | 「イベント」と「条件」（報酬＝チャンネルポイントの交換のみ／ユーザー／文面＝チャットの発言のみ。同じ種類は1件まで。すべてを満たしたときだけ動く）と、そのとき行う動作を並べて保存する。一覧では1行の要約に折りたたまれ、押すと入力欄が開く（開くのは1件ずつ）。動作は「アラートを出す」（素材・表示時間・音量・文言）・「チャットに送る」（文言）・「アナウンスを送る」（文言・色）の3種類で、1つだけでも複数でもよい。複数のトリガーが当てはまるイベントには、上にあるトリガーを使う |
