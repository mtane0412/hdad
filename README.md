...
- **行が足りない・長すぎる文言が返ってきた回は、補ったり切り詰めたりせず捨てて**、前の文言をそのまま出し続けます。枠からはみ出させず、見出しの無い片肺のテロップも出さないためです
...
- LLMが失敗したとき（無料枠を使い切った日など）や、**1行20文字・2行の上限を超える文言が返ってきたとき**は、切り詰めずに前の文言を出し続けます。失敗は収集の記録（`/api/admin/stats/failures` の `side-super-failed`）に残ります
