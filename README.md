...
どんなときに何をするか（トリガー）は、配信者がトリガーの管理画面（`/triggers/`）で決めます（「デプロイ」の節の「アラートの設定と素材」を参照）。トリガーは「きっかけ」と「動作」からなります。きっかけは**既定メニューから選びます**（イベント種別と条件を自分で組み合わせる形ではありません。このツールは汎用のノーコード自動化を目指していないためです）。メニューは「視聴者」「応援」「配信」の3つに分かれていて、項目によっては絞り込みを1つだけ選べます（報酬・ユーザー名・言葉・日数・自動で入った広告か）。
