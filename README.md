...
| `/triggers/` | トリガーの管理画面（アプリ）。既定メニューからきっかけを選んで何をするかを決め、アラート用オーバーレイのOBS用URLをコピーする |
...
| `/speech/` | チャットの読み上げの設定（アプリ）。設定を保存し、VOICEVOX に読ませるOBS用URLをコピーする |
...
| `speech/reader/` | チャットを VOICEVOX に読み上げさせるページ。映すものを持たず音だけを出す。単独のページ |
...
どんなときに何をするか（トリガー）は、配信者がトリガーの管理画面（`/triggers/`）で決めます（「デプロイ」の節の「アラートの設定と素材」を参照）。トリガーは「きっかけ」と「動作」からなります。きっかけは**最初から一覧に並んでいます**（イベント種別と条件を自分で組み合わせる形ではありません。このツールは汎用のノーコード自動化を目指していないためです）。配信者がすることは、効果を付けたい出来事を開いて何をするかを決めることだけです。一覧は「視聴者」「応援」「配信」の3つに分かれていて、項目によっては絞り込みを1つだけ選べます（報酬・ユーザー名・言葉・日数・自動で入った広告か）。**効果をひとつも付けていない出来事では何も起きません**（並んでいるだけなので、有効・無効の切り替えはありません）。

| 区分 | 項目 | 絞り込み |
|---|---|---|
| チャット | 初めて来た人の発言 | なし |
| チャット | 久しぶりの人の発言 | 前の発言から空いた日数（1〜365日） |
| チャット | その配信で最初の発言 | なし |
| チャット | すべての発言 | なし |
| チャット | 決まった言葉を含む発言 | 含まれる言葉（部分一致） |
| チャット | 決まった人の発言 | Twitchのユーザー名 |
| 応援 | チャンネルポイントが交換された | 報酬（既定はすべての報酬） |
| 応援 | フォローされた / サブスクされた（新規） / サブスクの継続メッセージが届いた / レイドされた | なし |
| 配信 | 広告が始まった / 広告が終わった | 自動で入った広告だけ・手動で打った広告だけ・どちらでも |

> **チャットの上3つは「挨拶」で、1通の発言に動くのは当てはまるうち一番上のものだけです。** 初めて来た人の発言は「その配信で最初の発言」でもあるので、すべて動かすと同じ1通に挨拶が二重・三重に飛んでしまいます。「すべての発言」は挨拶と**同時に**動くので、読み上げや効果音に向いています。

> **既定メニューにする前の設定は読み込めません。** イベント種別と条件を持つ古い形が保存されていると、管理画面もエラーになります（黙って読み替えないためです）。KVの `alert-config` を消してから、メニューで設定し直してください。

**報酬・言葉・ユーザーの3つは、1つの出来事の中に設定を複数持てます**（「乾杯」の報酬と「おみくじ」の報酬に別々の効果を付けられます）。ほかの出来事は1つずつです。効果には**アラートを出す**（このオーバーレイが再生する）・**チャットに送る**（Workerがbotとして送る）・**AIに文面を作らせて送る**（Workerがその人の記録を材料にLLMへ文面を作らせ、botとして送る）・**アナウンスを送る**（Workerがbotとして、モデレーター権限で色の付いた帯を送る）があり、レイドされたときだけ**シャウトアウトを送る**（Workerがbotとして、レイドしてきた配信者をTwitchのシャウトアウトで紹介する）も選べます。Twitchからの通知はすべてWorkerがWebhookで受け取り、当てはまったアラートだけをこのオーバーレイへWebSocket（`GET /api/overlay/socket`）で押し出します。**当てはまった行はすべて実行します**（1件だけを選ぶことはしません）。たとえば「誰かが発言した」と「初見さんが発言した」の両方に効果を付けていれば、初見さんの1通目では両方が動きます。アラートが重なったときは、届いた順に並べて1つずつ再生します。オーバーレイはTwitchへ直接つながず、届いたものを再生するだけです。判定を通知のたびに行うので、設定を変えてもOBSの再読み込みは不要です。どのトリガーに当てはまるかをWorkerが決めるのは、きっかけに「その配信で初めての発言か」「このチャンネルで初めての発言か」のようにデータベースの記録からしか決められないものがあるためです。アラートを出す動作を持つトリガーが1件もなければ何も表示しません。**オーバーレイを開いていない間に届いたアラートは貯めずに捨てます**（開いたときに過去の分がまとめて流れないようにするためです）。

チャットに送る動作・AIに文面を作らせて送る動作・アナウンスを送る動作はオーバーレイには渡らず、TwitchからWorkerへ直接届くWebhookで実行します。そのため**OBSを開いていなくてもチャットのお礼は送られます**（送るには「チャットボット」のページでbotアカウントを接続しておきます）。アナウンスとシャウトアウトはさらに、botがそのチャンネルのモデレーターにされている必要があります。

「チャット」の区分はすべて発言がきっかけなので、「決まった人が発言したら音を鳴らす」「合言葉が出たらアラートを出す」といった設定ができます（ユーザー名も言葉も、大文字小文字は区別しません）。挨拶の3項目は、次の発言に当てはまります。

| 項目 | 当てはまる発言 |
|---|---|
| その配信で最初の発言 | その配信で最初にチャットした人の1通目。同じ人の2回目以降の発言では動かず、配信が変わればまた動きます。**配信していないあいだの発言は初回と判定しません**（テスト配信や配信前の雑談のたびに動いてしまうため） |
| 初めて来た人の発言 | 視聴者の記録（`viewers`。「視聴者の記録」の節を参照）が1行もない人の1通目。Twitchの「初見」の印（IRCの `first-msg`）ではなく自前の記録で判定します。トリガーの判定に使うEventSubの通知にはこの印が無いためです。常連は当てはまらないので、「初めて来てくれた人にだけ挨拶する」ができます。**この記録を始める前から来ている常連も「初めて」と扱われます**（記録がないためです。運用を始めた直後は動作を控えめにしておくことをおすすめします） |
| 久しぶりの人の発言 | 最後の発言から指定した日数（1〜365日）以上空けて発言した人の1通目。「お久しぶりです」と声をかけるのに使います。このチャンネルで初めての人には当てはまりません（空いた日数が決まらないためです） |

「配信」の区分の**広告が始まった**・**広告が終わった**では、「広告が入ることをチャットで知らせる」「広告が終わったら声をかける」ができます。文言には広告の長さを差し込む `{duration}`（秒）が使えます。絞り込みで、自動広告だけ・手動で打った広告だけに絞れます（手で打った広告は配信者が自分で告知できるので、ふだん告知したいのは自動広告のほうです）。

> **Twitchには広告の終了に相当する通知がありません。** 届くのは開始（`channel.ad_break.begin`）だけで、そこに広告の長さ（`duration_seconds`）が入っています。そこでWorkerは開始を受けた時点で「終わる時刻」を Durable Object（`AD_BREAKS`。`worker/ad-break-timer.ts`）に預け、その時刻にアラームで起きて、擬似イベント `channel.ad_break.end` として同じ照合に回します。cron（5分おき）では30〜180秒の広告に間に合わず、Workerは接続もタイマーも持てないためです。終了のトリガーが1件もなければ預けません。
>
> **すでに動かしている場合は、配信者のログインし直しが必要です。** 広告の通知を購読するために、配信者のスコープに `channel:read:ads` が増えたためです。

後ろの2つは視聴者の記録から判定するので、**人ごとに記録を削除すると、その人はまた「初めて」に戻ります**。アラートを出す動作はbotの接続と関係なく動きますが、**チャットに送る・AIに文面を作らせて送る・アナウンスを送る・シャウトアウトを送る動作はbotを接続しているときだけ動きます**（送り主のアカウントが要るため）。未接続のときは `/triggers/` の画面で知らせます。
...
Twitchのチャットを、配信に使うPCで動かした [VOICEVOX](https://voicevox.hiroshiba.jp/) に読み上げさせるページです。映すものは持たず、音だけを出します。チャットはチャットボックスと同じ匿名IRCで受け取るのでTwitchのトークンは要りませんが、**読み上げの設定を Worker から読むのでオーバーレイ用キーが要ります**。読み上げに Workers AI を使わないのは、発言のたびに無料枠を消費させないためです。

```text
https://hdad.<サブドメイン>.workers.dev/speech/reader/?key=<オーバーレイ用キー>
```

**設定はURLではなく `/speech/` の画面で保存します。** 読み上げのページが30秒おきに読み直すので、配信中に変えても**OBSの再読み込みなしで次に読む1件から効きます**（ホストとポートを除く）。

| 設定 | 内容 |
| --- | --- |
| ホスト | VOICEVOX が動いているホスト。`localhost`（既定）か `127.0.0.1` のみ |
| ポート番号 | VOICEVOX ENGINE のポート番号（既定 `50021`） |
| 話者ID | 既定 `3` = ずんだもんのノーマル。VOICEVOX のキャラクターとスタイルの組み合わせ |
| 読み上げ速度 | `0.5`〜`2`（既定 `1`） |
| 音量 | `0`〜`1`（既定 `1`） |
| 読み上げる長さの上限 | 文字数。`1`〜`200`（既定 `60`） |
| 発言者の名前も読む | 有効にすると本文の前に表示名を読む（既定は無効） |
| 読み上げない人 | ログイン名をカンマ区切り。接続しているbotは、ボタン1つで足せます |

- **ホストとポートだけは、変えたらOBSでブラウザソースを再読み込みしてください**。つなぎ先が変わるのでつなぎ直しが要ります（画面にもその案内が出ます）
- 値の範囲の確認は Worker が行い、外れていれば保存せずに理由を返します
- **VOICEVOX を起動していないと動きません**。つながらないまま開くと、考えられる原因（起動していない・ポート番号が違う・CORSで拒まれている）と直し方を画面に出して止まります
- **VOICEVOX 側でこのサイトのオリジンを許可する必要があります**。ENGINE は既定（`--cors_policy_mode localapps`）では localhost・`app://`・ブラウザ拡張からの通信しか受け付けず、`https://hdad.<サブドメイン>.workers.dev` からの読み出しを拒みます。設定は次の手順です（この節の「OBSでの設定」の手順4も参照）
  1. 配信に使うPCのブラウザで `http://127.0.0.1:50021/setting` を開く
  2. CORSの許可するオリジンに `https://hdad.<サブドメイン>.workers.dev` を足して保存する
  3. **VOICEVOX を再起動する**（設定の適用に再起動が必要です）

  コマンドラインから ENGINE を起動している場合は `--allow_origin https://hdad.<サブドメイン>.workers.dev` でも同じです。なお `/transcript/` の中継ページが同じループバックへ設定なしでつながるのは、WebSocket が CORS の対象外だからで、こちらには同じ理屈が通りません
- 読まないもの: コマンド（`!` で始まる発言）・エモートやCheermoteだけの発言・「読み上げない人」に挙げた人の発言
- 読み替えるもの: URLは「URL」と読み、同じ文字が3つ以上続くところ（`wwwwww` など）は2つに縮めます。聞いても分からないものを読ませないためです
- **1件ずつ順に読みます**。チャットが速くて待ちが10件を超えたときは、古いほうから捨てて新しい発言を読みます（数分前の発言を読み続けても配信の役に立たないためです）
- 1件の読み上げに失敗しても止まりません（その1件を飛ばして次を読みます）。原因はブラウザソースのコンソールに残ります

### OBSでの設定

1. ソースの追加 > ブラウザ
2. URLに上のURL（`/speech/` の画面でコピーしたもの）を貼る。映すものが無いので大きさは任意
3. ソースのプロパティで**「OBSで音声を制御する」を有効にする**（配信に音を載せるため）
4. 配信に使うPCで VOICEVOX を起動し、`http://127.0.0.1:50021/setting` でこのサイトのオリジンをCORSの許可に足して、VOICEVOX を再起動しておく（上の箇条書きを参照）

## サイドスーパー（`side-super/`）

配信画面の隅（左上・右上）に、いま何をしているかを**2行のテロップ**で出し続けます。テレビ番組のサイドスーパーと同じく上下の役割が分かれていて、上の段は**コーナー名**（14文字まで。いまの配信が何の時間かを表す名前で、話題が少し動いても変わりません）、下の段は**いまの話題**（20文字まで）です。文言は配信中の文字起こし・視聴者の発言・配信のカテゴリとタイトルを材料に、Worker が cron（5分おき）で作って貯め、このオーバーレイが読み出して映します。利用には「デプロイ」の節の Twitchログインの設定が必要です。

```text
https://hdad.<サブドメイン>.workers.dev/side-super/overlay/?key=<オーバーレイ用キー>
```

| パラメータ | 内容 |
| --- | --- |
| `key` | オーバーレイ用キー（必須）。キーを含むURLはサイドスーパーの管理画面（`/side-super/`）でコピーできる（アラート・文字起こしと同じキーです） |
| `position` | `left`（既定。左上）か `right`（右上）。寄せる向きが変わる |
| `demo` | `true` ならWorkerに接続せず、サンプルの文言を順に流す（見栄えと配置の調整用）。このときは `key` が要りません |

- **作るのは cron（5分おき）**です。オーバーレイは30秒ごとに読みに行くだけで、LLMを呼びません（配信中ずっと開いていても Workers AI の無料枠を消費しません）
- **材料は直近のぶんだけ**で、あらすじ（「これまでのあらすじ」の節）と違って前回の文言に積み上げません。話題が移ったときに古い文言を引きずらないためです
- 文字起こし（`/transcript/`）の中継ページをOBSに置いていると、いまの話題をより正しく言い当てられます。置いていない場合の材料は視聴者の発言とカテゴリ・タイトルだけになります
- **前回作ったあとに新しい材料が1件も無ければ作り直しません**。喋りも発言もない時間帯に無駄にLLMを呼ばないためです
- 文言が作られるのを待たずに見栄えを確かめるには、`/side-super/` の画面の「デモを開く」か、URLの `?demo=true` を使います（サンプルの文言を順に流します。上限いっぱいの文字数のサンプルも入っているので、テロップがいちばん幅を取る場合も見られます）
- LLMが失敗したとき（無料枠を使い切った日など）や、**ちょうど2行でない・見出しが14文字を超える・本文が20文字を超える文言が返ってきたとき**は、行を補ったり切り詰めたりせず捨てて、前の文言をそのまま出し続けます（枠からはみ出させず、見出しの無い片肺のテロップも出さないためです）。失敗は収集の記録（`/api/admin/stats/failures` の `side-super-failed`）に残ります
- **配信していないあいだは何も映しません**（透過の枠だけが残ります）。配信の前後にOBSのソースを開いたままにして構いません
- 起動できない失敗（キーの誤りなど）は画面にエラーを出します。いっぽう2回目以降の読み出しの失敗は画面に出さず、前の文言を映したままにします（一時的な通信の失敗で配信画面を汚さないためです）

### OBSでの設定

1. ソースの追加 > ブラウザ
2. URLに上のURLを貼り、幅と高さを配信のキャンバスと同じ大きさ（1920×1080 など）にする。背景は透過で、文言は指定した側の上隅に表示される
3. 左上に出すか右上に出すかは、`/side-super/` の画面で選んでからURLをコピーする
...
| トリガー（`/triggers/`） | 「イベント」と「条件」（報酬＝チャンネルポイントの交換のみ／ユーザー／文面に含む言葉＝チャットの発言のみ、部分一致／その配信で初めての発言・このチャンネルで初めての発言・前の発言から空いた日数＝いずれもチャットの発言のみ／自動で入った広告か＝広告の開始・終了のみ。同じ種類は1件まで。すべてを満たしたときだけ動く）と、そのとき行う動作を並べて保存する。一覧では1行の要約に折りたたまれ、押すと入力欄が開く（開くのは1件ずつ）。動作は「アラートを出す」（素材・表示時間・音量・文言）・「チャットに送る」（文言）・「AIに文面を作らせて送る」（AIへの指示）・「アナウンスを送る」（文言・色）・「シャウトアウトを送る」（設定なし。レイドの項目だけに出る）の5種類で、1つだけでも複数でもよい（ただし「チャットに送る」と「AIに文面を作らせて送る」は同じ発言に2通返ってしまうので同時に選べない）。複数のトリガーが当てはまるイベントには、上にあるトリガーを使う |
...
アラートを出す動作は `durationSeconds` が1〜60、`volume` は0〜1、`message` は200文字以内（差し込み語が置き換わり、空文字なら文言を出さない）です。チャットに送る動作の `message` は1〜500文字（Twitchのチャット1通の上限）で、空文字は許しません。`{message}` を差し込んだ結果が500文字を超える場合は、末尾を `…` にして収めます（超えたままではTwitchが1通まるごと拒むため）。AIに文面を作らせて送る動作の `instruction`（AIへの指示）は1〜1000文字で、文面そのものではなく「どう書くか」を書きます（差し込み語は使いません）。同じ種類の動作は1つのトリガーに1件までで、`chat` と `aiChat` は同じトリガーに並べられません（同じ発言に2通返ってしまうため）。差し込み語はイベント種別ごとに違い、`{user}`（交換した人・フォローした人・レイド元の配信者・発言した人）のほか、`{reward}`・`{tier}`・`{months}`・`{viewers}`・`{message}`（発言の本文）・`{duration}`（広告の長さ。秒）が使えます。設定に問題があれば、保存せずに問題点の一覧（`error.problems`）を返します。

「AIに文面を作らせて送る」動作（`aiChat`）は、配信者が書いた指示と、相手の名前・発言の本文・その人の記録（`viewers` のメモ・AIが作った人物像・初回と最後の発言日時・おおよその発言数・最後に見たバッジ）と、「このチャンネルで初めての発言か」「前の発言から何日空いたか」を材料に、Workers AI（`wrangler.jsonc` の `ai` バインディング。モデルは `worker/ai-chat.ts` の `MODEL`）へ文面を作らせて送ります。視聴者の記録を材料に渡せるのはチャットの発言のトリガーだけで、ほかのイベントでは指示とイベントの中身だけから作らせます。「初めて」「お久しぶり」の判定は、トリガーの条件に書いていなくてもこの動作があれば行います（書かないと来訪の別を知らないまま文面を作らせることになるためです。そのぶん、発言のたびに視聴者の記録を読みます）。Webhookの応答を待たせるとTwitchが同じ通知を再送するため、**Twitchへ2xxを返してから**文面を作って送ります（`ExecutionContext.waitUntil`）。AIが作った文面が500文字（Twitchのチャット1通の上限）を超えたとき、AIが失敗したとき（Workers AI の無料枠は1日10,000 Neuronsで、00:00 UTC にリセットされます）は、**切り詰めたり固定文言に落としたりせず送るのをやめて** `collection_failures` に `alert-aichat-failed` として記録します。
...
botに要求するスコープは `user:bot`・`user:read:chat`・`user:write:chat` と、モデレーション操作のための `moderator:manage:banned_users`・`moderator:manage:chat_messages`・`moderator:manage:announcements`・`moderator:manage:shoutouts`（`worker/eventsub.ts` の `BOT_SCOPES`）です。配信者側には `channel:bot` と、botがモデレーターかどうかを確かめるための `moderation:read`、チャットの発言を配信者として読むための `user:read:chat` と `user:bot` が要ります。

> **すでに動かしている場合は、botの接続し直しと配信者のログインし直しの両方が必要です。** botのスコープに `moderator:manage:*`（シャウトアウトのための `moderator:manage:shoutouts` を含む）が、配信者のスコープに `moderation:read`・`user:read:chat` が増えたためです。足りないスコープは `/bot/` の画面に出ます。
...
| 応答文 | 送り返す文言。`{user}` が発言した人のログイン名に、`{summary}` がいまの配信の「これまでのあらすじ」（後述）に置き換わります。置き換わったあとで500文字まで |
...
配信の途中から来た人に、それまで何が話されていたかを伝えるための短い文です。応答文に `{summary}` を含むコマンド（たとえば `!summary` に「これまでのあらすじ: {summary}」）を登録すると、打たれたときにその文を返します。

- **材料**は、配信者が喋った内容（`/transcript/` の文字起こし。中継ページをOBSに置いている場合だけ集まります）と、配信中の視聴者の発言です。視聴者の名前は材料に含めません
- **作るのは cron（5分おき）**で、コマンドが打たれたときではありません。打つたびにLLMの応答を待たせないためです。そのため返るのは最大5分前の時点のあらすじです
- 毎回ゼロから作り直すのではなく、**前回のあらすじに、そのあとの出来事を積み上げて**書き直します。長い配信でも1回あたりのLLMへの入力が一定に保たれ、Workers AI の無料枠（1日10,000 Neurons）を食い潰しません
- **配信していないときは作りません**。あらすじは配信の区切りごとに1つ持つので、配信が変われば前の配信のあらすじは返りません（まだ無いときは「まだあらすじがありません」と返り、コマンドが無応答になることはありません）
- LLMが失敗したとき（無料枠を使い切った日など）は、前回のあらすじをそのまま返し続けます。失敗は収集の記録（`/api/admin/stats/failures` の `stream-summary-failed`）に残ります
...
このツールがLLMに文面を作らせる箇所は4つあります。**箇所ごとに**提供元とモデルを、管理画面（`https://<公開先のドメイン>/llm/`）で選べます。

| 使う箇所 | 何を作るか | 呼ばれる頻度 |
|---|---|---|
| チャットの文面 | トリガーの動作「AIチャット」が視聴者へ送る文面 | 当てはまった発言・イベントごと |
| サイドスーパー | 配信画面の隅に出す2行のテロップ | cron（5分おき） |
| 視聴者の人物像 | 配信が終わったあと、その人の発言からまとめた文 | cron（1回に5人ぶんまで） |
| 配信のあらすじ | 配信者の発話と視聴者の発言をまとめた文（`{summary}`） | cron（5分おき） |

選べる提供元は2つです。

| 提供元 | 用意するもの | 備考 |
|---|---|---|
| Workers AI（Cloudflare） | なし（Workerに付いています） | 既定。無料枠（1日10,000 Neurons）の中で動きます |
| OpenRouter | シークレット `OPENROUTER_API_KEY` | ほかの会社のモデルも選べます。料金はOpenRouterのアカウントに請求されます |

「あらすじだけ OpenRouter の賢いモデルに任せ、発言ごとに呼ばれるチャットの文面は無料枠の Workers AI に留める」といった使い分けができます。既定は4か所すべて Workers AI で、あらすじだけ大きいモデル（`llama-3.3-70b`）、ほかの3か所は軽いモデル（`llama-3.1-8b`）です。あらすじに大きいモデルを充てるのは、小さいモデルでは視聴者の書き込みを配信者がしたことのように書く・同じ言い回しを繰り返して長さの上限を超える、といった壊れ方が起きたためです。

OpenRouter を使うときは、先に鍵を設定してください。

```bash
# https://openrouter.ai/keys で発行した鍵を設定する
...
その人がどんな人かの要約（人物像）は、AIが作って記録に足します。材料はその人がその配信で話した内容なので、**配信中のあいだだけはチャットの本文も貯めます**（`stream_chat_messages`）。これは「発言そのものは貯めない」という上の方針の、意識して設けた例外です（発言数やバッジだけでは人物像を作れないためです）。貯めた本文は、配信が終わったあとに cron（5分おき）が1回5人ずつ人物像にまとめ、**使い終えたその場で消します**。人物像を作るときは前回までの人物像も材料に渡すので、配信を重ねるほど積み上がります。作った人物像は、次にその人が発言したときの「AIに文面を作らせて送る」動作（`aiChat`）の材料にもなります。AIが失敗したとき（無料枠切れを含む）は人物像を作らず、`collection_failures` に `viewer-summary-failed` として記録し、材料は次の収集でやり直せるよう残します（残ったまま7日を過ぎた材料は cron が消します）。記録を削除すると、まだ人物像にしていない本文も、配信中のぶんまで含めて一緒に消えます。
...
| 人物像 | 配信が終わったあとにAIが作るもの（200文字まで）。**メモとは別の列に持ち、画面でも別々に出します**（機械の推測と人が書いたものを混ぜないためです）。画面からは書き換えられません |
...
- この記録は、トリガーの「このチャンネルで初めての発言」「前の発言から空いた日数」の条件の判定にも使います。判定は発言を記録したあとに行い、**同じ通知が再送されても答えが変わりません**（記録を作った発言のIDと、更新する前の最後の発言日時を控えているためです）
...
| `stream_chat_messages` | 人物像の材料になる、配信中のチャットの本文。配信が終わったあとに cron が人物像へまとめ、使い終えた行をその場で消す。残った行も7日で消す |
| `collection_failures` | 収集の失敗（30日分） |

1回の収集ではこのほかに、終わった配信の発言から視聴者の人物像を作ります（1回5人まで。「視聴者の記録」の節を参照）。
...
- 届いた通知は、記録に加えてアラートのトリガーにかけます。当てはまるトリガーが「アラートを出す」動作を持っていれば、アラートをオーバーレイへ押し出します（botの接続は要りません）。「チャットに送る」「AIに文面を作らせて送る」「アナウンスを送る」動作を持っていれば、botとしてチャットへ1通送ります。「シャウトアウトを送る」動作を持っていれば、botとしてシャウトアウトを1件送ります（いずれもbotが未接続なら何もしません）。同じ通知が再送されても二度は実行せず、失敗した場合もTwitchへは2xxを返して `collection_failures` に `alert-push-failed`・`alert-chat-failed`・`alert-announce-failed`・`alert-aichat-failed`・`alert-shoutout-failed` として記録します（2xx以外だとTwitchが再送し、実行できていた場合に二重になるため）。広告の終了は Durable Object のアラームから実行するので、その予約の失敗は `ad-break-end-schedule-failed`、アラームが鳴ってからの失敗は `ad-break-end-failed` として記録します（予約を消したあとなので、再試行では取り返せません）
