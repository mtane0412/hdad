...
1. botに使うTwitchアカウントを用意する（配信者と同じアカウントでも動きますが、チャット欄での見分けがつくよう別アカウントを勧めます）
2. 配信者としてログインし、`https://<公開先のドメイン>/bot/` を開く
3. 「botアカウントを接続する」を押し、**botアカウントで**Twitchの認可画面を通る（ブラウザのTwitchのログインを切り替えるか、シークレットウィンドウを使ってください）
4. 接続後、同じ画面の「テスト送信」で、チャットへ実際に送れることを確かめる
5. モデレーター権限が要る操作を後で足す場合は、Twitchのチャットから配信者が `/mod <botのログイン名>` で権限を与えます
