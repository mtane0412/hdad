...
| `viewers` | チャットで発言した人を1人1行持つ（初回と最後の発言日時・通算の発言数・最後に見たバッジ・配信者のメモ）。発言そのものは貯めない。古い行を消す仕組みは無く、ずっと残す。「このチャンネルで初めての発言」「前の発言から空いた日数」の条件の判定にも使う |
