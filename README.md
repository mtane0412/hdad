...
ページUI（ダッシュボード・ギャラリー・トリガーとアップロードの管理画面）は、配信者のTwitchログインを前提にしたサイドバー付きのアプリです（`npm run dev` の中でWorkerも動くので、開発サーバーでもログインできます）。トップの `index.html` ひとつでできていて、`/wallpaper/` のようなパスに応じた中身をアプリが描きます。ログインしていなければ、どのパスを開いてもログインの入口だけが出ます。OBSに載せる素材ページはパスごとの実ファイルで、ログインなしで動きます。素材はカテゴリごとにディレクトリを分けて置きます。

| パス | 内容 |
| --- | --- |
| `/` | ダッシュボード（アプリ） |
| `/wallpaper/`・`/clock/`・`/chat/` | 各カテゴリのギャラリー（アプリ）。素材を選び、パラメータを調整して、OBS用のURLをコピーする |
| `/triggers/` | トリガーの管理画面（アプリ）。どのイベントで何をするかを決め、アラート用オーバーレイのOBS用URLをコピーする |
| `/media/` | アップロードの画面（アプリ）。アラートで流す画像・動画・音声を置く |
| `/bot/` | チャットボットの管理画面（アプリ）。botアカウントの接続・切断、テスト送信、コマンドと自動モデレーションの設定 |
| `/viewers/` | 視聴者の記録（アプリ）。チャットで発言した人の一覧と、配信者が書くメモ |
...
| パラメータ | 内容 |
| --- | --- |
| `key` | オーバーレイ用キー（必須）。キーを含むURLはトリガーの管理画面（`/triggers/`）でコピーできる。Twitchのトークンではないので、漏れてもTwitchアカウントには影響しない |
| `demo` | `true` でサンプルのアラートを一定間隔で流す（配置の調整用。Twitchには接続しない） |

どのイベントで何をするか（トリガー）は、配信者がトリガーの管理画面（`/triggers/`）で決めます（「デプロイ」の節の「アラートの設定と素材」を参照）。トリガーは「イベント種別」と「条件（報酬・ユーザー・発言の文面・その配信で初めての発言かで絞る。すべてを満たしたときだけ動く。条件なしならそのイベントでいつでも動く）」と「動作」からなり、動作には**アラートを出す**（このオーバーレイが再生する）・**チャットに送る**（Workerがbotとして送る）・**アナウンスを送る**（Workerがbotとして、モデレーター権限で色の付いた帯を送る）があります。Twitchからの通知はすべてWorkerがWebhookで受け取り、当てはまったアラートだけをこのオーバーレイへWebSocket（`GET /api/overlay/socket`）で押し出します。オーバーレイはTwitchへ直接つながず、届いたものを再生するだけです。判定を通知のたびに行うので、設定を変えてもOBSの再読み込みは不要です。どのトリガーに当てはまるかをWorkerが決めるのは、条件に「その配信で初めての発言か」のようにデータベースの記録からしか決められないものがあるためです。アラートを出す動作を持つトリガーが1件もなければ何も表示しません。**オーバーレイを開いていない間に届いたアラートは貯めずに捨てます**（開いたときに過去の分がまとめて流れないようにするためです）。
...
イベント種別には**チャットの発言**（`channel.chat.message`）も選べるので、「特定の人が発言したら音を鳴らす」「合言葉が出たらアラートを出す」といった設定ができます。発言を絞るには「ユーザー」の条件（発言者のTwitchのユーザー名）・「文面に含む言葉」の条件（本文にその言葉を含む発言に当てはまる。大文字小文字は区別しない部分一致）・「その配信で初めての発言」の条件を使います。最後のものを使うと「その配信で最初にチャットした人に音を鳴らす」ができます。同じ人の2回目以降の発言では動かず、配信が変わればまた動きます。**配信していないあいだの発言は初回と判定しません**（テスト配信や配信前の雑談のたびに動いてしまうため）。アラートを出す動作はbotの接続と関係なく動きますが、**チャットに送る・アナウンスを送る動作はbotを接続しているときだけ動きます**（送り主のアカウントが要るため）。未接続のときは `/triggers/` の画面で知らせます。

キーの指定漏れのように起動できない失敗は画面全体にエラーを表示し、切断などの一時的な失敗は左下にお知らせを出してつなぎ直します。つながらないまま閉じた場合（キーの誤りなど）も、ブラウザからは理由を読めないため、左下に手がかりを出したうえでつなぎ直し続けます。
...
トリガーの管理画面（`https://<公開先のドメイン>/triggers/`）とアップロードの画面（`/media/`）で、配信者としてログインして行います。

| 欄 | できること |
|---|---|
| OBS用のURL（`/triggers/`） | オーバーレイ用キーを含むURLのコピー（配信画面に映り込んでも読めないよう伏せ字で表示する）と、キーの再発行。再発行すると今のURLは使えなくなる |
| 素材（`/media/`） | 画像・動画・音声のアップロード（1ファイル50MBまで）、試し見・試し聴き、削除。トリガーに使われている素材は削除できない |
| トリガー（`/triggers/`） | 「イベント」と「条件」（報酬＝チャンネルポイントの交換のみ／ユーザー／文面に含む言葉＝チャットの発言のみ、部分一致／その配信で初めての発言＝チャットの発言のみ。同じ種類は1件まで。すべてを満たしたときだけ動く）と、そのとき行う動作を並べて保存する。一覧では1行の要約に折りたたまれ、押すと入力欄が開く（開くのは1件ずつ）。動作は「アラートを出す」（素材・表示時間・音量・文言）・「チャットに送る」（文言）・「アナウンスを送る」（文言・色）の3種類で、1つだけでも複数でもよい。複数のトリガーが当てはまるイベントには、上にあるトリガーを使う |

管理用API（`/api/admin/*`）は配信者のセッションが必要で、書き換えを伴うメソッドは管理画面と同じサイトからのリクエスト（`Origin` ヘッダーが一致するもの）だけを受け付けます。ローカルでは `npm run dev`（`http://localhost:5173/triggers/`）で確かめます。`@cloudflare/vite-plugin` が開発サーバーの中でWorkerを動かします。

アラートを出す動作は `durationSeconds` が1〜60、`volume` は0〜1、`message` は200文字以内（差し込み語が置き換わり、空文字なら文言を出さない）です。チャットに送る動作の `message` は1〜500文字（Twitchのチャット1通の上限）で、空文字は許しません。`{message}` を差し込んだ結果が500文字を超える場合は、末尾を `…` にして収めます（超えたままではTwitchが1通まるごと拒むため）。同じ種類の動作は1つのトリガーに1件までです。差し込み語はイベント種別ごとに違い、`{user}`（交換した人・フォローした人・レイド元の配信者・発言した人）のほか、`{reward}`・`{tier}`・`{months}`・`{viewers}`・`{message}`（発言の本文）が使えます。設定に問題があれば、保存せずに問題点の一覧（`error.problems`）を返します。
...
チャットで発言した人を、Worker が1人1行でD1（`viewers`）に貯めます。**発言そのものは貯めません**（チャットは件数の桁が違い、配信の記録と書き込みの枠を食い合うため）。貯めるのは人なので、数千人でも数MBに収まり、消さずに持ち続けられます。「この人は前にも来てくれた」「前はこういう話をした」を配信者が振り返るためのものです。

視聴者の画面（`https://<公開先のドメイン>/viewers/`）では、最後に発言した順に並ぶ一覧を見て、ログイン名の前方一致で絞り込み、人ごとにメモを書けます。一覧は50人ずつ読み、続きは「もっと読み込む」で足します。

| 記録する値 | 内容 |
|---|---|
| ログイン名・表示名 | 最後に見た名前。本人がいつでも変えられるので、**照合にはTwitchのユーザーIDだけを使います**（名前で照合すると、改名した人が別人として増えます） |
| 初回・最後の発言日時 | このチャンネルで初めて発言した日時と、最後に発言した日時 |
| 通算の発言数 | 後述のとおり10分に1回までしか数えないので、実際の発言数より少なくなります |
| 最後に見たバッジ | 発言の通知に含まれるバッジ（配信者・モデレーター・VIP・サブスク）。**サブスクやVIPの状態は自前で管理しません**。Twitchを真実の持ち主として、最後に観測した値を控えるだけにします（自前で持つと、Twitch側での変更と必ず食い違います） |
| メモ | 配信者が手で書くもの（2000文字まで） |

- 同じ人の記録を更新するのは、**前回から10分が経ったときだけ**です。それより短い間隔で届いた発言では1行も書き込みません（条件に合わない行はD1の書き込みとして数えられないため、常連が連投しても枠を食いません）
- **bot自身の発言は記録しません**。自動モデレーションで処分した発言は記録します（荒らしの履歴も配信者には有用なためです）
- 同じ通知がTwitchから再送されても、発言数は二重に増えません（最後に記録した発言のIDを控えています）。ただし控えているのは直前の1件だけなので、10分の間引きで記録しなかった発言がそれより後に再送された場合は、1回数えられます。発言数はもともと数え落とすことを承知のうえでの概数です
- 記録は人ごとに削除できます（本人から求められたときに応じるためのものです）。削除した人がまた発言すれば、初めての人として記録し直します
...
| `viewers` | チャットで発言した人を1人1行持つ（初回と最後の発言日時・通算の発言数・最後に見たバッジ・配信者のメモ）。発言そのものは貯めない。古い行を消す仕組みは無く、ずっと残す |
...
- 届いた通知は、記録に加えてアラートのトリガーにかけます。当てはまるトリガーが「アラートを出す」動作を持っていれば、アラートをオーバーレイへ押し出します（botの接続は要りません）。「チャットに送る」「アナウンスを送る」動作を持っていれば、botとしてチャットへ1通送ります（botが未接続なら何もしません）。同じ通知が再送されても二度は実行せず、失敗した場合もTwitchへは2xxを返して `collection_failures` に `alert-push-failed`・`alert-chat-failed`・`alert-announce-failed` として記録します（2xx以外だとTwitchが再送し、実行できていた場合に二重になるため）
