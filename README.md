...
どのイベントで何をするか（トリガー）は、配信者がトリガーの管理画面（`/triggers/`）で決めます（「デプロイ」の節の「アラートの設定と素材」を参照）。トリガーは「イベント種別」と「条件（報酬・ユーザー・発言の文面を絞る。すべてを満たしたときだけ動く。条件なしならそのイベントでいつでも動く）」と「動作」からなり、動作には**アラートを出す**（このオーバーレイが再生する）・**チャットに送る**（Workerがbotとして送る）・**アナウンスを送る**（Workerがbotとして、モデレーター権限で色の付いた帯を送る）があります。オーバーレイは通知が届くたびに最新の設定を取得するので、設定を変えてもOBSの再読み込みは不要です。アラートを出す動作を持つトリガーが1件もなければ何も表示しません。
...
イベント種別には**チャットの発言**（`channel.chat.message`）も選べるので、「特定の人が発言したら音を鳴らす」「合言葉が出たらアラートを出す」といった設定ができます。発言を絞るには「ユーザー」の条件（発言者のTwitchのユーザー名）と「文面に含む言葉」の条件（本文にその言葉を含む発言に当てはまる。大文字小文字は区別しない部分一致）を使います。アラートを出す動作は、オーバーレイが配信者のトークンで発言を購読するのでbotの接続と関係なく動きますが、**チャットに送る・アナウンスを送る動作はbotを接続しているときだけ動きます**（Workerが発言を受け取る購読の条件に、botのユーザーIDが要るため）。未接続のときは `/triggers/` の画面で知らせます。
...
| トリガー（`/triggers/`） | 「イベント」と「条件」（報酬＝チャンネルポイントの交換のみ／ユーザー／文面に含む言葉＝チャットの発言のみ、部分一致。同じ種類は1件まで。すべてを満たしたときだけ動く）と、そのとき行う動作を並べて保存する。一覧では1行の要約に折りたたまれ、押すと入力欄が開く（開くのは1件ずつ）。動作は「アラートを出す」（素材・表示時間・音量・文言）・「チャットに送る」（文言）・「アナウンスを送る」（文言・色）の3種類で、1つだけでも複数でもよい。複数のトリガーが当てはまるイベントには、上にあるトリガーを使う |
