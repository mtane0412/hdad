...
どのイベントで何をするか（トリガー）は、配信者がトリガーの管理画面（`/triggers/`）で決めます（「デプロイ」の節の「アラートの設定と素材」を参照）。トリガーは「イベント種別」と「条件（報酬・ユーザー・発言の文面・その配信で初めての発言かで絞る。すべてを満たしたときだけ動く。条件なしならそのイベントでいつでも動く）」と「動作」からなり、動作には**アラートを出す**（このオーバーレイが再生する）・**チャットに送る**（Workerがbotとして送る）・**アナウンスを送る**（Workerがbotとして、モデレーター権限で色の付いた帯を送る）があります。オーバーレイは通知が届くたびにWorker（`POST /api/overlay/alert`）へ問い合わせて再生するものを決めるので、設定を変えてもOBSの再読み込みは不要です。どのトリガーに当てはまるかをWorkerが決めるのは、条件に「その配信で初めての発言か」のようにデータベースの記録からしか決められないものがあるためです。アラートを出す動作を持つトリガーが1件もなければ何も表示しません。
...
| `POST /api/overlay/alert?key=` | オーバーレイが受け取った通知に対して、再生するアラート（素材のURL・表示時間・音量・文言）を返す。当てはまるトリガーがなければ `alert` は `null`（要オーバーレイ用キー） |
...
| `first_chatters` | 配信の区切りごとに、その配信で最初に発言した人を1行持つ（「その配信で初めての発言」の条件の判定に使う）。1日より古い行は cron が消す |
